...
package me.hgj.jetpackmvvm.demo.app.network.Utils;
...
public class Utils {
    ...
    public static RequestBody createFile(String name) {
        ...
        return RequestBody.create(okhttp3.MediaType.parse("multipart/form-data; charset=utf-8"),
                name);
    }
    ...
    public static <T> Type findNeedClass(Class<T> cls) {
        ...
        Type finalNeedType;
        if (params == null) {
            finalNeedType = new TypeToken<Object>() {
            }.getType();
        } else {
            type = params[0];
    ...
    public static <T> Type findRawType(Class<T> cls) {
        ...
        Type returnType;
        try {
            returnType = getGenericType((ParameterizedType) genType, 0);
        } catch (Exception e) {
            e.printStackTrace();
            returnType = new TypeToken<Object>() {
            }.getType();
        }
        return returnType;
    }
