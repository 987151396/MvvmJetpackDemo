...
class RequestTodoViewModel : BaseViewModel() {
    ...
    fun addTodo(todoTitle: String, todoContent: String, todoTime: String, todoLeve: Int) {
        ...
        params.put("title",todoTitle)
        params.put("content",todoContent)
        params.put("date",todoTime)
        params.put("type","0")
        params.put("priority",todoLeve.toString())

        request({
            MeHttpHelper.instance.post("/lg/todo/add/json",params,Any::class.java)
        }, {
